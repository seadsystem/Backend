...
class TestQueryRouting(unittest.TestCase):
	...
	def test_total_energy(self):
		db.generate_total_energy = lambda device_id, start_time, end_time, device: 157
		self.assertEqual(
			db.query({'device_id': 4, 'total_energy': True}),
			157
		)
	...
	def test_classify(self):
		...
		db.retrieve_within_filters = self.retrieve_factory()
		Analysis_3.run = self.classify_factory()

		self.assertEqual(db.query(self.query_options), 'classify called')
	...
	def test_events(self):
		...
		db.format_list = self.format_list_factory('detect called', 'event')

		self.assertEqual(db.query(self.query_options), 'format_list called: event')
	...
	def test_bare_format_list(self):
		...
		db.retrieve_within_filters = self.retrieve_factory()
		db.format_list = self.format_list_factory('retrieve called', True)

		self.assertEqual(db.query(self.query_options), 'format_list called: True')
	...
	def test_format_data(self):
		...
		db.retrieve_within_filters = self.retrieve_factory()
		db.format_list = self.format_list_factory('retrieve called', True)
		db.format_data = self.format_data_factory(['time', 'I', 'W', 'V', 'T'], 'retrieve called', True)

		self.assertEqual(db.query(self.query_options), 'format_data called')
...
class TestQueryGeneration(unittest.TestCase):
	...
	def test_device_id(self):
		...
		db.perform_query = self.format_list_factory(
		...
		self.assertEqual(db.retrieve_within_filters(*self.retrieve_args), 'perform_query called')
